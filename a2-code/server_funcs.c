...
client_t *server_get_client(server_t *server, int idx) {
  check_fail(idx > server->n_clients, 1, "index greater than client range");
  client_t *selected_client = &server->client[idx];
...
void server_start(server_t *server, char *server_name, int perms) {
  ...
  snprintf(server_name_fifo, sizeof(server_name_fifo), "%s.fifo", server_name);
  int fifo_chk = mkfifo(server_name_fifo, perms); //perms is iffy
  check_fail(fifo_chk != 0, 1, "failed to create fifo");

  int join_server_fd = open(server_name_fifo, O_RDWR);
  ...
  strncpy(server->server_name, server_name_fifo, sizeof(server_name_fifo));
...
void server_shutdown(server_t *server) {
	...
	unlink(server->server_name);		//unlink the join fifo or remove instead?????????
	server_broadcast(server, &mesg);	//sending the BL_SHUTDOWN message to all clients
...
int server_remove_client(server_t *server, int idx) {
	...
	{
    client_t *client_p = server_get_client(server, idx);
		close(client_p->to_client_fd);	//close client fifos
		close(client_p->to_server_fd);
		unlink(client_p->to_client_fname);	//remove client fifos
		unlink(client_p->to_server_fname);
		for (int i = idx + 1; i < server->n_clients; i++)
		{
			server->client[idx] = server->client[i];
			idx = i;
		}
	server->n_clients-=1;
	}
...
int server_broadcast(server_t *server, mesg_t *mesg) {
	...
	{
		int bytes = write(server->client[i].to_client_fd, mesg, sizeof(mesg));
		check_fail(bytes != sizeof(mesg_t), 1,
               "Write to client fifo %s did not complete", server->client[i]);
	}
	return 0;
...
void server_check_sources(server_t *server) {
  ...
  for(int i = 0; i < server->n_clients; i++) {
    curr_client_fd = server->client[i].to_server_fd;
    FD_SET(curr_client_fd, &read_set);
  ...
  */
  ...
  int error = select(maxfd + 1, &read_set, NULL, NULL, NULL);
  if(error == -1) {
    if(errno == 4){
      perror("sent an interrupt");
      return;
    }
  ...
  for(int j = 0; j < server->n_clients; j++) {
    if(FD_ISSET(server->client[j].to_server_fd, &read_set)) {
      server->client[j].to_server_fd = 1;
    }
  }
...
int server_handle_join(server_t *server) {
  ...
  strncpy(join_message.body, "", sizeof(""));
...
int server_handle_client(server_t *server, int idx) {
	mesg_t mesg;
  client_t *client_p = server_get_client(server, idx);
	int bytes = read (client_p->to_server_fd, &mesg.body,
                    sizeof(mesg_t));
	check_fail(bytes != sizeof(mesg_t), 1, "Message read from client failed.");
	...
	if(mesg.kind == BL_DEPARTED)
		{
			if (strncmp(mesg.body, "", sizeof(mesg.body)) != 0)
			{
        strncpy(mesg.body, "", sizeof(""));
			}
      server_remove_client(server, idx);
			server_broadcast(server, &mesg);
		}
	client_p->data_ready = 0;
 	return 0;
